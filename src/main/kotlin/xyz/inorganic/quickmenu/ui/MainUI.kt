...
class MainUI : Screen(Component.translatable("menu.main.title")) {
    ...
    override fun extractRenderState(graphics: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        if (actions.isEmpty()) {
            ...
            graphics.centeredText(font, emptyMsg, menuX + menuWidth / 2, menuY + menuHeight / 2, 0x66FFFFFF.toInt())
        }
    ...
    override fun keyPressed(event: KeyEvent): Boolean {
        ...
        if (ModKeybindings.closeSearchKeybind.matches(event) && isSearching) {
            isSearching = false
            if (::searchBox.isInitialized) searchBox.value = ""
            rebuildWidgets()
            return true
